...
(function () {
    ...
    // --- Variáveis para animação / marca d'água ---
    ...
    // --- Função mostrarBanner incluída para evitar erro ---
    ...
    // --- Marca d'água animada no canto inferior direito com texto e emoji ---
    function adicionarIconeMarcaDagua() {
        if (iconeMarcaDagua) return; // prevenir múltiplas inserções
    ...
    // --- Função helper para log visual e no console, usando emojis e estilo ---
    ...
    // --- Espera até que um seletor apareça no DOM (máx timeout) ---
    ...
    for (const sigla of Object.keys(resultados)) {
        ...
        // Garantir que os valores são strings/inteiros ou vazio caso indefinidos
        const linha = [
            sigla,
            base.totalNaBase ?? '',
            base.comAtraso ?? '',
    ...
    // --- Criar botão para iniciar extração ---
    // Alterações para o botão criarBotaoExtrair() - apenas a função modificada:
...
function criarBotaoExtrair() {
    ...
    Object.assign(btn.style, {
        position: 'fixed',
        bottom: '60px',
        right: '35px',      // alterado de 15px para 25px para alinhar sobre o ícone
        zIndex: 10001,
        width: '24px',
        height: '24px',
        padding: '0',
    ...
    async function main() {
        ...
        console.log('%c🚀 Extrator Arco Loggi Plus iniciado', 'color: #25D366; font-weight: bold; font-size: 16px;');
        console.log('%cClique no botão verde no canto inferior direito para iniciar a extração.', 'color: #666; font-size: 13px;');
    }

    // API pública opcional para controle externo
